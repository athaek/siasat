...
<table border="1" cellspacing="0">
    ...
    $tahun_akademik_id=  getField('student_siswa', 'angkatan_id', 'nis', $biodata['nis']);
    // konsentrasi
    $konsentrasi_id=getField('student_siswa', 'konsentrasi_id', 'nis', $biodata['nis']);
    $no=1;
    $sisa_total=0;
        foreach ($jenis_bayar as $jb)
        {
            $jumlah_bayar   =(int) get_biaya_sekolah($tahun_akademik_id, $jb->jenis_bayar_id, $konsentrasi_id, 'jumlah');
            $sudah_bayar    = (int)get_biaya_sudah_bayar($biodata['nis'], $jb->jenis_bayar_id);
        ...
        for($i=1;$i<=$semester;$i++)
        {
            ...
            $spp_udah_bayar =   (int)get_semester_sudah_bayar($biodata['nis'], $i);
