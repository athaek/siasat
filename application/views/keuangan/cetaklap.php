...
<h3>LAPORAN KEUANGAN SMA NEGERI 1 TUBAN</h3>
...
<table border="1" cellspacing="0" bordercolor="#7E7E7E">
    <tr><th rowspan="2">No</th><th rowspan="2">NIS</th><th rowspan="2" width="150">NAMA</th>
    <?php
    ...
    $no=1;
    $tunggakan_total=0;
    $siswa=$this->db->query("select nis,nama from student_siswa where konsentrasi_id='$konsentrasi_id' and angkatan_id='$t->angkatan_id'")->result();
    foreach ($siswa as $m)
    {
        echo "<tr><td>$no</td>
            <td>".  strtoupper($m->nis)."</td>
        ...
        foreach ($jenis_bayar->result() as $j)
        {
            $jml_bayar=  chek_bayar($m->nis, $j->jenis_bayar_id, 01);
            $sdh_bayar=  chek_bayar($m->nis, $j->jenis_bayar_id, 02);
...
<Br>
<?php
$jml_semester=$this->db->query("SELECT max(semester_aktif) as max_semester FROM `student_siswa` WHERE konsentrasi_id='$konsentrasi_id' and angkatan_id='$t->angkatan_id'")->row_array();
    ...
    for($i=1;$i<=$jml_semester;$i++)
{
        ...
        $sdh_bayar=chek_bayar_semester($m->nis, $i);
...
<table border="1" cellspacing="0" bordercolor="#7E7E7E">
    ...
    $no=1;
    $tunggakan_total=0;
    $siswa=$this->db->query("select nis,nama from student_siswa where konsentrasi_id='$konsentrasi_id' and angkatan_id='$tahun_akademik'")->result();
    foreach ($siswa as $m)
    {
        echo "<tr><td>$no</td>
            <td>".  strtoupper($m->nis)."</td>
...
<Br>
<?php
$jml_semester=$this->db->query("SELECT max(semester_aktif) as max_semester FROM `student_siswa` WHERE konsentrasi_id='$konsentrasi_id' and angkatan_id='$tahun_akademik'")->row_array();
$jml_semester=$jml_semester['max_semester'];
?>
<table border="1" cellspacing="0" bordercolor="#7E7E7E">
    <tr><th>No</th><th>NIM</th><th>NAMA</th>
