...
<table class="table table-bordered">
    ...
    <tr><td width="120">NIS Siswa</td><td> <?php echo inputan('text', 'nis','col-sm-6','Masukan Nis ..', 1, '','');?> <input type="submit" value="OK" name="submit" class="btn btn-danger"> <?php echo anchor('keuangan/reset','Reset',array('class'=>'btn btn-danger'));?></td>
    ...
    <tr><td>Kelas /Konsentrasi</td><td> : <?php echo $statuss=="kosong"?"":strtoupper($profile['nama_konsentrasi'].' / '.$profile['nama_kelas'])?></td></tr>
...
<table class="table table-bordered">
    ...
    $tahun_akademik_id=  getField('student_siswa', 'angkatan_id', 'nis', $nis);
    // konsentrasi
    $konsentrasi_id=getField('student_siswa', 'konsentrasi_id', 'nis', $nis);
    $no=1;
        foreach ($jenis_bayar as $jb)
        {
            $jumlah_bayar   =(int) get_biaya_sekolah($tahun_akademik_id, $jb->jenis_bayar_id, $konsentrasi_id, 'jumlah');
            $sudah_bayar    = (int)get_biaya_sudah_bayar($nis, $jb->jenis_bayar_id);
       ...
       // get semester aktif
       $smt_aktif = getField('student_siswa', 'semester_aktif', 'nis', $nis);
        // looping semester
        for($i=1;$i<=$smt_aktif;$i++)
        {
            $spp            =   (int) get_biaya_sekolah($tahun_akademik_id, 3, $konsentrasi_id, 'jumlah');
            $spp_udah_bayar =   (int)get_semester_sudah_bayar($nis, $i);
...
<table class="table table-bordered">
    ...
    ?> 
    <tr><td colspan="7"><?php echo anchor('keuangan/cetakpersonaldetail','Cetak Data',array('class'=>'btn btn-danger','target'=>'new'))?></td></tr>
</table>
