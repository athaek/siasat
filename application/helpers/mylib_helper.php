...
if ( ! function_exists('generatehtml'))
{
    ...
    function get_biaya_sekolah($tahun_akademik,$jenis_biaya_sekolah,$konsentrasi,$field)
    {
        ...
        $r      =  $CI->db->get_where('keuangan_biaya_sekolah',$where);
        if($r->num_rows()>0)
        {
            $r=$r->row_array();
            return $r[$field];
    ...
    function get_biaya_sudah_bayar($nis,$jenis_bayar_id)
    {
        $CI     =   & get_instance();
        $query  =   "SELECT sum(jumlah) as jumlah 
                    from keuangan_pembayaran_detail 
                    where nis='$nis' and jenis_bayar_id='$jenis_bayar_id' 
    ...
    function get_semester_sudah_bayar($nis,$semester)
    {
        $CI     =   & get_instance();
        $query  =   "SELECT sum(jumlah) as jumlah 
                    from keuangan_pembayaran_detail 
                    where nis='$nis' and jenis_bayar_id='3' and semester='$semester' 
                    group by jenis_bayar_id";
        $data   =   $CI->db->query($query);
        if($data->num_rows()>0)
        {
    ...
    function chek_bayar($nis,$jenis_bayar,$kode)
    {
        // 01 jumlah harus bayar dan 02 jumlah yang sudah dibayar
        $CI     =   & get_instance();
        $m      =   $CI->db->query("select konsentrasi_id,angkatan_id from student_siswa where nis='$nis'")->row_array();
        if($kode==01)
        {

            $j=$CI->db->get_where('keuangan_biaya_sekolah',array( 'jenis_bayar_id'=>$jenis_bayar,'angkatan_id'=>$m['angkatan_id'],'konsentrasi_id'=>$m['konsentrasi_id']))->row_array();
            return  $j['jumlah'];
        }
        else
        {
            $sql="SELECT sum(jumlah) as total from keuangan_pembayaran_detail where nis='$nis' and jenis_bayar_id=$jenis_bayar";
    ...
    function chek_bayar_semester($nis,$semester)
    {
        $CI     =   & get_instance();
    ...
    function jml_spp_konsentrasi($konsentrasi_id,$tahun_akademik_id)
    {
        ...
        $data=$CI->db->get_where('keuangan_biaya_sekolah',array('jenis_bayar_id'=>3,'konsentrasi_id'=>$konsentrasi_id,'angkatan_id'=>$tahun_akademik_id));
        if($data->num_rows()>0)
        {
            $r=$data->row_array();
            return $r['jumlah'];
    ...
    function status_registrasi($tahun_akademik_id,$nis,$field)
    {
        $CI     =   & get_instance();
        $data=$CI->db->get_where('akademik_registrasi',array('nis'=>$nis,'tahun_akademik_id'=>$tahun_akademik_id));
        if($data->num_rows()<0)
        {
            return '';
        }
    ...
    function users_keterangan($level,$keterangan)
    {
        if($level==2)
        {
            ...
            return getField('akademik_kelas', 'nama_kelas', 'kelas_id', $keterangan);
        }
        elseif($level==3)
        {
            return getField('app_guru', 'nama_lengkap', 'guru_id', $keterangan);
        }
        else
        {
            return '';
    ...
    function akses_guru()
    {
        $CI     =   & get_instance();
        $sess   =   $CI->session->userdata('level');
        $guru  =   $CI->session->userdata('keterangan');
        if($sess==3)
        {
            // chek id ada atau tidak
            $chek   =$CI->db->get_where('app_guru',array('guru_id'=>$guru))->num_rows();
            if($chek<1)
            {
                redirect('auth/login');
            }
    ...
    function chek_jadwal_sekolah($konsentrasi,$hari,$tahun_akademik,$semester,$no)
    {
        $CI     =   & get_instance();
        $sql="  SELECT jk.jam_mulai,jk.jam_selesai,ah.hari,mm.nama_mapel,ar.nama_ruangan,ad.nama_lengkap
                FROM akademik_jadwal_sekolah as jk,app_hari as ah,mapel_matapelajaran as mm,app_ruangan as ar,app_guru as ad 
                WHERE jk.hari_id=ah.hari_id and mm.mapel_id=jk.mapel_id and ar.ruangan_id=jk.ruangan_id and ad.guru_id=jk.guru_id 
        ...
        {
            $r=$data->row_array();
            return $r['nama_mapel'].'<br>'.  strtoupper($r['nama_lengkap']).'<br><b>'.  strtoupper($r['hari']).', '.$r['jam_mulai'].' - '.$r['jam_selesai'].'</b><br>RUANGAN '.  strtoupper($r['nama_ruangan']);
        }
        else
        {
            return '';
