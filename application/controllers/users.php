...
class users extends CI_Controller{
    ...
    function keterangan($id)
    {
        ...
        {
            return getField('akademik_kelas', 'nama_kelas', 'kelas_id', $id);
        }
    }
    ...
    function post()
    {
        if(isset($_POST['submit']))
        {
            ...
            if($level==2)
            {
                 $data   =   array('username'=>$username,'password'=>md5($password),'level'=>$level,'keterangan'=>$kelas);
            }
            else
            {
    ...
    function account()
    {
        ...
        if(isset($_POST['submit']))
        {
            ...
            $data           =   array(  'nama_lengkap'=>$nama,
                                        ...
                                        'alamat'=>$alamat,'hp'=>$hp,
                                        'email'=>$email);
            $this->mcrud->update('app_guru',$data, 'guru_id',$id);
            redirect('users/account');
        }
        else
        {
            $data['title']=  $this->title;
            $data['r']   =  $this->mcrud->getByID('app_guru',  'guru_id',  $id)->row_array();
