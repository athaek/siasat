...
class keuangan extends CI_Controller
{
    ...
    function view()
    {
        ...
        $query="SELECT sm.nama,sm.nis,jb.keterangan,pd.tanggal,pd.jumlah,pd.semester,ak.nama_konsentrasi
                FROM keuangan_pembayaran_detail as pd,keuangan_jenis_bayar as jb,student_siswa as sm,akademik_konsentrasi as ak
                WHERE pd.jenis_bayar_id=jb.jenis_bayar_id and ak.konsentrasi_id=sm.konsentrasi_id and sm.nis=pd.nis and left(pd.tanggal,10) 
    ...
    function tampilkandata2($konsentrasi,$tahun_angkatan)
    {
        ...
        echo   "<table class='table table-bordered'>
                <tr><th rowspan='2' width='7'>No</th><th width='16' rowspan='2'>NIS</th>
        ...
        echo   "</tr>";
        $no=1;
        $siswa =  $this->db->get_where('student_siswa',array('konsentrasi_id'=>$konsentrasi,'angkatan_id'=>$tahun_angkatan))->result();
        foreach ($siswa as $m)
        {
            echo"<tr><td>$no</td><td>$m->nis</td><td>$m->nama</td>";
               // data bayaran
                foreach ($jenis_bayar->result() as $j)
                {
                    $harus_bayar=chek_bayar($m->nis, $j->jenis_bayar_id, 01);
                    $sudah_bayar=chek_bayar($m->nis, $j->jenis_bayar_id, 02);
    ...
    {
        // fires else
            $data=  $this->db->get_where('student_siswa',array('konsentrasi_id'=>$konsentrasi,'angkatan_id'=>$tahun_akademik))->result();
            $jenis_bayar=  $this->db->get('keuangan_jenis_bayar');
            ...
            echo"<table class='table table-bordered'>
                <tr><th rowspan=2>No</th><th rowspan=2>Nis</th><th rowspan='2' width='200'>Nama Lengkap</th>";
            foreach ($jenis_bayar->result() as $j)
            {
            ...
            foreach ($data as $r)
            {
                ...
                echo"<tr bgcolor='$bgcolor'>
                    ...
                    {
                        $harus_bayar=chek_bayar($r->nis, $jb->jenis_bayar_id, 01);
                        $sudah_bayar=chek_bayar($r->nis, $jb->jenis_bayar_id, 02);
                        $sisa1=$harus_bayar-$sudah_bayar;
                        $sisa=$sisa1==0?'Lunas':rp((int) $sisa1);
                        echo "<td>".  rp((int)$harus_bayar)."</td>
                            <td>".  rp((int)$sudah_bayar)."</td>
                            <td>".  $sisa."</td>";
                        $tunggakanssw=$tunggakanssw+$sisa1;
                        //
                    }
                    echo"<td>".rp((int) $tunggakanssw)."</td></tr>";
                    $tunggakan=$tunggakan+$tunggakanssw;
                    $no++;
            }
            ...
            echo"<tr class='success'>
                ...
                <td colspan=3>Total Tunggakan</td><td><b>".rp((int)$tunggakan)."</b></td>
                    ...
                    // hitung sudah berapa tahun sekolah berdasarkan tahun akademik
            $tahun_sekarang=date('Y');
            $jml_semester=($tahun_akademik-$tahun_sekarang)+1;
            $smt_aktif= getField('student_siswa', 'semester_aktif', 'angkatan_id', $_GET['tahun_angkatan']);
            ...
            foreach ($data as $r)
            {
                ...
                // looping kolom bayar
                    for($i=1;$i<=$smt_aktif;$i++)
                    {
                        $spp=  chek_bayar_semester($r->nis, $i);
    ...
    function pembayaran()
    {
        if(isset($_POST['submit']))
        {
            ...
            $chek   = $this->db->get_where('student_siswa',array('nis'=>$nis))->num_rows();
            if($chek>0)
            {
            $this->session->set_userdata('pembayaran_siswa_nis',$nis);
            }
            else
            {
                $this->session->set_flashdata('pesan', "<div class='alert alert-success'><i class='fa fa-bullhorn'></i> NIS YANG ANDA MASUKAN TIDAK DITEMUKAN DI DATABASE</div>");
            }
        ...
        elseif(isset($_POST['submit2']))
        {
            ...
            $idnis=$this->session->userdata('pembayaran_siswa_nis');
            $tahun_akademik = getField('student_siswa', 'angkatan_id', 'nis', $idnis);
            $konsentrasi_id = getField('student_siswa', 'konsentrasi_id', 'nis', $idnis);
            $semester_aktif = getField('student_siswa', 'semester_aktif', 'nis', $idnis);
            $jumlah_bayar   = get_biaya_sekolah($tahun_akademik, $jenis, $konsentrasi_id, 'jumlah');
            $sudah_bayar    = get_biaya_sudah_bayar($idnis, $jenis);
            ...
            // jika selain spp chek sudah lunas atau belum
             ...
             if($jenis==3)
             {
                 if($semester>$semester_aktif)
                 {
                    ...
                    $this->session->set_flashdata('pesan', "<div class='alert alert-danger'><i class='fa fa-bullhorn'></i> SEMESTER YANG ANDA INPUTKAN TIDAK SESUAI DENGAN DATA SISWA</div>");
                 }
                 else
                 {
                     // chek spp semester itu udah lunas belum
                    $sdh_bayar_semester= $this->chek_sudah_bayar_semester($idnis, $semester);
                    ...
                    $data   =   array(  'jenis_bayar_id'=>$jenis,
                                        ...
                                        'tanggal'=>  waktu(),
                                        'semester'=>$semester,
                                        'nis'=>$this->session->userdata('pembayaran_siswa_nis'));
        ...
        {
            $nis_session=$this->session->userdata('pembayaran_siswa_nis');
            $query2="   SELECT au.nama,kj.keterangan,kd.tanggal,kd.jumlah,kd.pembayara_detail_id,kd.jenis_bayar_id,kd.semester
                        ...
                        WHERE kd.jenis_bayar_id=kj.jenis_bayar_id and kd.id_users=au.id_users and kd.nis='$nis_session'";
            $query=     "SELECT sm.nama,ak.nama_konsentrasi,ap.nama_kelas
                        FROM student_siswa as sm,akademik_konsentrasi as ak,akademik_kelas as ap
                        WHERE sm.konsentrasi_id=ak.konsentrasi_id and ap.kelas_id=ak.kelas_id and sm.nis='$nis_session'";
            $data['transaksi']=  $this->db->query($query2)->result();
            $data['profile']=  $this->db->query($query)->row_array();
            if($nis_session=="emptyy")
            {
                $data['statuss']="kosong";
            }
            else
            ...
            $data['nis']=$nis_session;
            $data['semester']=getField('student_siswa', 'semester', 'nis', $nis_session);
    ...
    function chek_sudah_bayar_semester($nis,$semester)
    {
        $sql=   "SELECT sum(jumlah) as jumlah 
                FROM keuangan_pembayaran_detail
                WHERE jenis_bayar_id=3 and nis='$nis' and semester='$semester'";
    ...
    function reset()
    {
        $this->session->set_userdata('pembayaran_siswa_nis','emptyy');
        redirect('keuangan/pembayaran');
    }

    function cetakpersonal()
    {
        $session=$this->session->userdata('pembayaran_siswa_nis');
        if($session=="")
        {
            redirect($this->index());
        }
        ...
        {
            $query="SELECT sm.nis,sm.nama,ak.nama_konsentrasi,ta.keterangan
                    FROM student_siswa as sm,akademik_konsentrasi as ak,student_angkatan as ta
                    WHERE sm.konsentrasi_id=ak.konsentrasi_id and ta.angkatan_id=sm.angkatan_id and sm.nis='$session'";
            $nis_session=$this->session->userdata('pembayaran_siswa_ni');
            $data['jenis_bayar']=  $this->db->query('select * from keuangan_jenis_bayar where jenis_bayar_id not in("3")')->result();
            $data['biodata']=  $this->db->query($query)->row_array();
            $data['semester']=getField('student_siswa', 'semester_aktif', 'nis', $nis_session);
    ...
    function laporanpembayaran()
    {
        ...
        $query="SELECT sm.nama,sm.nis,jb.keterangan,pd.tanggal,pd.jumlah,pd.semester,ak.nama_konsentrasi
                FROM keuangan_pembayaran_detail as pd,keuangan_jenis_bayar as jb,student_siswa as sm,akademik_konsentrasi as ak
                WHERE pd.jenis_bayar_id=jb.jenis_bayar_id and ak.konsentrasi_id=sm.konsentrasi_id and sm.nis=pd.nis and pd.tanggal 
    ...
    function jurnalkhusus()
    {
        if(isset($_POST['submit']))
        {
            $data['tanggal1']=$this->input->post('tanggal1');
            $data['tanggal2']=$this->input->post('tanggal2');
            $query="SELECT sm.nis,sm.nama,jb.keterangan as jenis_bayar,pd.tanggal,pd.jenis_bayar_id,pd.jumlah
                    FROM keuangan_pembayaran_detail as pd,student_siswa as sm,keuangan_jenis_bayar as jb
                    WHERE sm.nis=pd.nis and pd.jenis_bayar_id=jb.jenis_bayar_id and left(pd.tanggal,10) BETWEEN '".$data['tanggal1']."' and '".$data['tanggal2']."' ";
            $data['transaksi']=  $this->db->query($query)->result();
    ...
    function cetakjurnalkhusus()
    {
        ...
        $query="SELECT sm.nis,sm.nama,jb.keterangan as jenis_bayar,pd.tanggal,pd.jenis_bayar_id,pd.jumlah
                    FROM keuangan_pembayaran_detail as pd,student_siswa as sm,keuangan_jenis_bayar as jb
                    WHERE sm.nis=pd.nis and pd.jenis_bayar_id=jb.jenis_bayar_id 
    ...
    function sms()
    {
        ...
        foreach ($tahun_akademik as $t)
        {
            ...
            {
                // siswa
                $siswa=  $this->db->get_where('student_siswa',array('angkatan_id'=>$t->angkatan_id,'konsentrasi_id'=>$k->konsentrasi_id))->result();
                foreach ($siswa as $m)
                {
                    $tunggakan_persiswa=0;
                    $tunggakan=0;
                    $jenis_bayar=  $this->db->get('keuangan_jenis_bayar')->result();
                    foreach ($jenis_bayar as $j)
                    {
                        // HB = harus bayar & SB = Sudah bayar
                        $hb     =  chek_bayar($m->nis, $j->jenis_bayar_id, 01);
                        $sb     =  chek_bayar($m->nis, $j->jenis_bayar_id, 02);
                        $sisa   =   $hb-$sb;
                    ...
                    for($i=1;$i<=$semester;$i++)
                    {
                        ...
                        $sdh_bayarspp   = chek_bayar_semester($m->nis, $i);
    ...
    function pdf()
    {
        ...
        $pdf->Cell(25,14,'NIS',1,0,'L');
        $pdf->Cell(83,14,'Nama Siswa',1,0,'L');
